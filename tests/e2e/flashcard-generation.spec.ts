...
test.describe("Flashcard Generation Flow", () => {
  ...
  const SAMPLE_TEXT =
    ...
    Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.`.repeat(
      2
    );
  ...
  test("should generate and show flashcard proposals", async ({ page }) => {
    ...
    const responsePromise = page.waitForResponse(
      (response) => response.url().includes("/api/generations") && response.status() === 201
    );
    ...
    await flashcardProposalPage.expectFlashcardContent(
      0,
      MOCK_FLASHCARDS.flashcards_proposal[0].front,
      MOCK_FLASHCARDS.flashcards_proposal[0].back
    );
  });
  ...
  test("should save accepted flashcards", async ({ page }) => {
    ...
    const generationsPromise = page.waitForResponse(
      (response) => response.url().includes("/api/generations") && response.status() === 201
    );
    ...
    const flashcardsPromise = page.waitForResponse(
