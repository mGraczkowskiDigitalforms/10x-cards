...
test.describe('Flashcard Generation Validation', () => {
  ...
  test('should validate minimum text length', async ({ page, dashboardPage }) => {
    ...
    await expect(page.locator('[data-test-id="characters-needed"]'))
      .toContainText(`(Need ${TEXT_VALIDATION.MIN_LENGTH - shortText.length} more)`);
  });
  ...
  test('should validate maximum text length', async ({ page, dashboardPage }) => {
    ...
    await expect(dashboardPage.generateButton).toBeDisabled();
    await expect(page.locator('[data-test-id="generate-error-alert"]'))
      .toContainText(`Text cannot be longer than ${TEXT_VALIDATION.MAX_LENGTH} characters`);
  });
