...
export function GenerateView({ userId }: GenerateViewProps) {
  ...
  const handleAccept = (index: number) => {
    setProposals((prev) => prev.map((p, i) => (i === index ? { ...p, accepted: true, rejected: false } : p)));
  };

  const handleEdit = (index: number, front: string, back: string) => {
    setProposals((prev) => prev.map((p, i) => (i === index ? { ...p, front, back, edited: true } : p)));
  };

  const handleReject = (index: number) => {
    setProposals((prev) => prev.map((p, i) => (i === index ? { ...p, rejected: true, accepted: false } : p)));
  };
  ...
  return (
    <div className="space-y-8">
      <TextInputForm onSubmit={handleGenerate} disabled={loading} shouldReset={shouldResetForm} />

      {loading && <LoadingIndicator />}

      {error && <ErrorAlert errorMessage={error} onDismiss={() => setError(null)} />}
